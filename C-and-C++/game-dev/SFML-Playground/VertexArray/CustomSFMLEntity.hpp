...
class MyEntity : public sf::Drawable, public sf::Transformable {
    ...
    MyEntity(float radius, sf::Color color = sf::Color::White) {
        m_vertices.setPrimitiveType(sf::TrianglesStrip);
        m_vertices.resize(4);
        m_vertices.append({{0.f, 0.f}, color});
        m_vertices.append({{radius, 0.f}, color});
        m_vertices.append({{radius / 3, radius / 2}, color});
        m_vertices.append({{(radius / 3) + radius / 3, radius / 2}, color});
    }

    MyEntity(float radius, sf::Texture &texture) : MyEntity(radius) {
        m_texture = texture;
    }
